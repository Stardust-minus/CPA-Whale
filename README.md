...
- Windows 原生客户端：v0.3.0
- CLIProxyAPI 动态插件：v0.3.0
- Linux 部署管理工具：v0.3.0
...
- `quota`：只有已实现 adapter 且实际观察到兼容信号的账户才可用
...
CI 定义见 [`.github/workflows/ci.yml`](.github/workflows/ci.yml)：每次 push/PR 都运行测试并上传三平台构建产物。推送与 workspace 版本一致的 tag（例如 `v0.3.0`）会触发 [`.github/workflows/release.yml`](.github/workflows/release.yml)，再次执行测试、ABI/PE/secret 检查并发布 GitHub Release、版本化二进制、bundle、manifest 和 checksums。
